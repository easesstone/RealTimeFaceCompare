...
object KMeansClustering {
  ...
  def main(args: Array[String]) {
    ...
    for (i <- region_ipcMap) {
      ...
      k_wsseMap.map(data =>
        (data._1, math.abs((a * data._1 - data._2 + b)) / math.sqrt(math.pow(a, 2) + 1))).foreach(println(_))*/ val kMeansModel = KMeans.train(trainData, numClusters, numIterations)
      val trainMidResult = kMeansModel.predict(idPointRDD.map(_._2))
      var trainResult = trainMidResult.zip(joinData.select("id", "time", "ipc", "host", "spic", "bpic").rdd).groupByKey().sortByKey().map(data => (data._1, data._2.toArray.sortWith((a, b) => a.getTimestamp(1).getTime > b.getTimestamp(1).getTime)))

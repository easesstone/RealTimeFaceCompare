...
object KafkaToEs {
  ...
  def setupSsc(topics: Set[String], kafkaParams: Map[String, String]
    ...
    val zKPaths: String = getItem("job.zkPaths", properties)
    ...
    kafkaDF.foreachRDD(rdd => {
      ...
      rdd.foreachPartition(parData => {
        ...
        parData.foreach(data => {
          ...
          if (status != 1) {
            LOG.error("Put data to es failed! And the failed ftpurl is " + data._2)
          }
        })
      })
    })

...
LOG_FILE=${CLUSTER_LOG_DIR}/sparkKMeansClusteringJob.log
######## common目录 ########
...
## spark class
SPARK_CLASS_PARAM=com.hzgc.cluster.clustering.KMeansClustering
## bigdata cluster path
BIGDATA_CLUSTER_PATH=/opt/hzgc/bigdata
# spark conf path
SPARK_CONF_PATH=${BIGDATA_CLUSTER_PATH}/Spark/spark/conf
#spark jars path
SPARK_JARS_PATH=${BIGDATA_CLUSTER_PATH}/Spark/spark/jars
...
MYSQL_VERSION=mysql-connector-java-5.1.38.jar
SPARK_INDEXEDRDD_VERSION=spark-indexedrdd-0.4.0.jar
PART_VERSION=part_2.10-0.1.jar
...
if [ ! -e ${CLUSTER_LIB_DIR}/${MYSQL_VERSION} ];then
    echo "${CLUSTER_LIB_DIR}/${MYSQL_VERSION} does not exit!"
    exit 0
else
    echo "copy 文件 ${MYSQL_VERSION} 到 spark/jars......"  | tee  -a  ${LOG_FILE}
    cp ${CLUSTER_LIB_DIR}/${MYSQL_VERSION} ${SPARK_JARS_PATH}
    echo "copy完毕......"  | tee  -a  ${LOG_FILE}
fi
...
${CLUSTER_LIB_DIR}/${SPARK_INDEXEDRDD_VERSION},\
${CLUSTER_LIB_DIR}/${PART_VERSION},\

...
import com.hzgc.hbase.staticrepo.{ObjectInfoHandlerImpl, ObjectInfoInnerHandlerImpl}
...
object FaceRecognizeAlarmJob {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val args = Array("172.18.18.100:21005,172.18.18.101:21005,172.18.18.102:21005", "testface")
    // Usage
    if (args.length < 2) {
      System.err.println(
        s"""
    ...
    kafkaFeature.foreachRDD(rddMessage => {
      rddMessage.foreachPartition(partitionMessage => {
        val parEsClient = new ObjectInfoInnerHandlerImpl
        val parWarnClient = new DeviceUtilImpl
        val parMQProducer = RocketMQProducer.getInstance()
        partitionMessage.foreach(message => {
          val ipcID = message._1
          val parWarnRule = parWarnClient.isWarnTypeBinding(ipcID)
          if (parWarnClient.getplatfromID(ipcID) != null &&
            parWarnRule != null &&
            parWarnRule.containsKey(DeviceTable.IDENTIFY)) {
            val parTypeList = parWarnRule.get(DeviceTable.IDENTIFY)
            ...
            if (tempMessage.length > 0) {
              ...
              val mqRD = finalMessage.iterator
              ...
              while (mqRD.hasNext) {
                ...
                val sim = element.sim
                val item = new Item()
                if (parTypeList.containsKey(staticObjectType)) {
                  parEsClient.updateObjectInfoTime(staticID)
                }
              ...
              parMQProducer.send(platID, DeviceTable.IDENTIFY.toString, "alarmInfo", gson.toJson(alarmMessage).getBytes(), null)
            }
          }
        })
      })

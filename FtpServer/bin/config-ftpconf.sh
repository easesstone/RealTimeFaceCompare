...
function config_ftpAddress()
{
	...
	FTP_SERVICEIPS=$(grep ftp_serviceip ${CONF_FILE}|cut -d '=' -f2)
...
function config_rkmq()
{
	...
	ROCKET_NAMESERVER=$(grep rocketmq_nameserver ${CONF_FILE}|cut -d '=' -f2)
    # 替换ftpAddress.properties中FTP安装节点IP值：key=value（替换key字段的值value）
...
function config_pdcrOverFtp()
{
	...
	KAFKA_HOSTS=$(grep kafka_installnode ${CONF_FILE}|cut -d '=' -f2)
    # 将这些分号分割的ip用放入数组
...
function config_users()
{
    ...
    FTP_DATA_PATH=$(grep FTP_DataDir ${CONF_FILE}|cut -d '=' -f2)

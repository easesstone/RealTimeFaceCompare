...
package com.hzgc.ftpserver.config.spring;
...
public class ServerBeanDefinitionParser extends
    ...
    protected void doParse(final Element element,
        ...
        List<Element> childs = com.hzgc.ftpserver.config.spring.SpringUtil.getChildElements(element);
        for (Element childElm : childs) {
            ...
            if ("listeners".equals(childName)) {
                ...
                factoryBuilder.addPropertyValue("userManager", com.hzgc.ftpserver.config.spring.SpringUtil
                ...
                factoryBuilder.addPropertyValue("fileSystem", com.hzgc.ftpserver.config.spring.SpringUtil
                        .parseSpringChildElement(childElm, parserContext,
                                builder));
        ...
        if (StringUtils.hasText(element.getAttribute("max-logins"))) {
            connectionConfig.setMaxLogins(com.hzgc.ftpserver.config.spring.SpringUtil.parseInt(element,
                    "max-logins"));
        }
        if (StringUtils.hasText(element.getAttribute("max-threads"))) {
            connectionConfig.setMaxThreads(com.hzgc.ftpserver.config.spring.SpringUtil.parseInt(element,
                    "max-threads"));
        }
        if (StringUtils.hasText(element.getAttribute("max-anon-logins"))) {
            connectionConfig.setMaxAnonymousLogins(com.hzgc.ftpserver.config.spring.SpringUtil.parseInt(element,
                    "max-anon-logins"));
        }
        if (StringUtils.hasText(element.getAttribute("anon-enabled"))) {
            connectionConfig.setAnonymousLoginEnabled(com.hzgc.ftpserver.config.spring.SpringUtil.parseBoolean(
                    element, "anon-enabled", true));
        }
        if (StringUtils.hasText(element.getAttribute("max-login-failures"))) {
            connectionConfig.setMaxLoginFailures(com.hzgc.ftpserver.config.spring.SpringUtil.parseInt(element,
                    "max-login-failures"));
        }
        if (StringUtils.hasText(element.getAttribute("login-failure-delay"))) {
            connectionConfig.setLoginFailureDelay(com.hzgc.ftpserver.config.spring.SpringUtil.parseInt(element,
                    "login-failure-delay"));
        }
    ...
    private Map<?, ?> parseFtplets(final Element childElm,
        ...
        List<Element> childs = com.hzgc.ftpserver.config.spring.SpringUtil.getChildElements(childElm);
        ...
        if(childs.size() > 0 && childs.get(0).getLocalName().equals("map")) {
            ...
            for (Element ftpletElm : childs) {
                ftplets
                        .put(ftpletElm.getAttribute("name"), com.hzgc.ftpserver.config.spring.SpringUtil
    ...
    private Map<?, ?> parseListeners(final Element listenersElm,
        ...
        List<Element> childs = com.hzgc.ftpserver.config.spring.SpringUtil.getChildElements(listenersElm);

...
rm -rf /opt/hzgc/bigdata/Spark/spark/conf/ftpAddress.properties
rm -rf /opt/hzgc/bigdata/Spark/spark/conf/hbase-site.xml
...
cp $CONF_DIR/ftpAddress.properties  /opt/hzgc/bigdata/Spark/spark/conf
cp $CONF_DIR/hbase-site.xml  /opt/hzgc/bigdata/Spark/spark/conf
...
if [ ! -e $LIB_DIR/hbase-common-1.0.2.jar ];then
    echo "$LIB_DIR/hbase-common-1.0.2.jar does not exit!"
    exit 0
fi
...
if [ ! -e $LIB_DIR/hbase-protocol-1.0.2.jar ];then
    echo "$LIB_DIR/hbase-protocol-1.0.2.jar does not exit!"
    exit 0
fi
if [ ! -e $LIB_DIR/ftpserver-core-1.0.6.jar ];then
    echo "$LIB_DIR/ftpserver-core-1.0.6.jar does not exit!"
    exit 0
fi
...
if [ ! -e /opt/hzgc/bigdata/HBase/hbase/lib/hbase-shaded-miscellaneous-1.0.1.jar ];then
    echo "/opt/hzgc/bigdata/HBase/hbase/lib/hbase-shaded-miscellaneous-1.0.1.jar does not exit!"
    exit 0
fi
if [ ! -e $LIB_DIR/rocketmq-1.5.0.jar ];then
    echo "$LIB_DIR/rocketmq-1.5.0.jar does not exit!"
    exit 0
fi
...
if [ ! -e $LIB_DIR/streaming-1.5.0.jar ];then
    echo "$LIB_DIR/streaming-1.5.0.jar does not exit!"
    exit 0
fi
if [ ! -e $CONF_DIR/es-config.properties ];then
    echo "$CONF_DIR/es-config.properties does not exit!"
...
## 开启识别告警任务
source /etc/profile
nohup /opt/hzgc/bigdata/Spark/spark/bin/spark-submit \
--master yarn-cluster \
--executor-memory 5g \
--executor-cores 4 \
--class com.hzgc.cluster.alarm.FaceRecognizeAlarmJob \
--jars $LIB_DIR/gson-2.8.0.jar,\
$LIB_DIR/hbase-client-1.0.2.jar,\
$LIB_DIR/hbase-common-1.0.2.jar,\
$LIB_DIR/jackson-core-2.8.6.jar,\
$LIB_DIR/spark-streaming-kafka_2.11-1.6.2.jar,\
$LIB_DIR/hbase-1.5.0.jar,\
$LIB_DIR/hbase-server-1.0.2.jar,\
$LIB_DIR/jni-1.5.0.jar,\
$LIB_DIR/hbase-protocol-1.0.2.jar,\
$LIB_DIR/ftpserver-core-1.0.6.jar,\
$LIB_DIR/kafka_2.11-0.8.2.1.jar,\
$LIB_DIR/elasticsearch-1.0.jar,\
$LIB_DIR/ftp-1.5.0.jar,\
$LIB_DIR/bigdata-api-1.5.0.jar,\
/opt/hzgc/bigdata/HBase/hbase/lib/hbase-shaded-miscellaneous-1.0.1.jar,\
$LIB_DIR/rocketmq-1.5.0.jar,\

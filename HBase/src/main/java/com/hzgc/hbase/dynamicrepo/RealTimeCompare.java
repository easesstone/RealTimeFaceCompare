...
public class RealTimeCompare implements Serializable {
    ...
    private static List<String> deviceIds;//设备Id列表
    private static String platformId;//平台Id
    private static Date startDate;//开始日期
    private static Date endDate;//结束日期
    private static List<TimeInterval> intervals;
    ...
    static {
        SparkConf conf = new SparkConf().setAppName("RealTimeCompare").setMaster("local[4]");
    ...
    private SearchResult compareByImage(PictureType pictureType, SearchOption option) {
        ...
        if (null != searchFea && searchFea.length == 512) {
            ...
            System.out.print(queryImageId);
            //将图片特征插入到特征库
            boolean insertStatus = dynamicPhotoService.upPictureInsert(pictureType, queryImageId, searchFea, image);
            if (insertStatus) {
                LOG.info("insert feature to HBase success");
            } else {
    ...
    private SearchResult compareByImageId(PictureType pictureType, SearchOption option) {
        ...
        if (null != fea && fea.length == 2048) {
            ...
            try {
                ...
                LOG.error("get feature failed by imageId", e);
            }
            if (null != imageIdList && imageIdList.size() > 0) {
                //对特征进行比对
                imageIdSimTupList = featureCompare(searchFea, imageIdFeaTupList);
    ...
    private SearchResult lastResult(final List<Tuple2<String, Float>> imageIdSimTup, final float threshold, final int type, String sortParams) {
        ...
        //保存到Hbase
        boolean flag = dynamicPhotoService.insertSearchRes(searchResult.getSearchId(), imgSimilarityMap);
        if (flag) {
            LOG.info("The search history of: " + searchId + " saved success");
            System.out.println("查询记录保存成功");
        } else {
            LOG.error("The search history of: " + searchId + " saved failed");
